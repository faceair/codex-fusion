...
Codex 默认不会主动开 subagent——即使打开了 multi-agent，默认模式仍是 `ExplicitRequestOnly`（除非用户或 AGENTS.md 明确要求才 spawn）。codex-fusion 通过 `usage_hint_text` 配置注入一段 developer 授权，让 fusion 把主动委派当作非平凡任务的默认路径，不用等用户明确要求。`usage_hint_text` 只注入给 primary agent，不会泄漏给 subagent。

同时用 `agents.max_depth = 1` + multi-agent v1 模式做系统层硬约束：root session（depth 0）可以 spawn subagent（depth 1），但 subagent 不能再 spawn——v2 模式的 spawn handler 会忽略 `max_depth`，v1 模式会强制检查并返回 `"Agent depth limit reached"`。
